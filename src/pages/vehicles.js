...
export function saveVehicle(editId) {
  ...
  if (!data.regNumber || !data.name || !data.maxLoad) {
    ...
    document.getElementById('vehicle-error-msg').textContent = 'Please fill all required fields';
...
export function exportVehicles() {
  ...
  const headers = ['Reg Number', 'Model', 'Type', 'Max Load', 'Odometer', 'Acq. Cost', 'Status', 'Region'];
  const rows = vehicles.map(v => [v.regNumber, v.name, v.type, v.maxLoad, v.odometer, v.acquisitionCost, v.status, v.region]);
