  ...
  return `
    <div class="page-header">
      ...
      <div class="page-actions">
        <button class="btn btn-ghost btn-sm" onclick="window.app.exportReportCSV()">
    ...
    <div class="table-container">
      <table class="data-table">
        <thead>
          <tr>
            <th>Vehicle</th><th>Fuel Eff.</th><th>Revenue</th>
            <th>Fuel Cost</th><th>Maint. Cost</th><th>Total Cost</th><th>ROI</th>
          </tr>
        </thead>
        <tbody>
          ${vehicles.map(v => {
    ...
    const roi = v.acquisitionCost > 0
      ? (((rev - costs.total) / v.acquisitionCost) * 100).toFixed(1)
      : 0;
    const roiColor = Number(roi) >= 0 ? 'var(--accent-green)' : 'var(--accent-red)';
    return `<tr>
...
export function initReportCharts() {
  import('chart.js').then(({ Chart, registerables }) => {
    ...
    const otherTotal = store.getExpenses()
    ...
    if (ctx1) {
      new Chart(ctx1, {
        ...
        data: {
          ...
          datasets: [{ data: [fuelTotal, maintTotal, otherTotal], backgroundColor: ['#34d399', '#fbbf24', '#60a5fa'], borderWidth: 0, spacing: 3 }]
        },
    ...
    if (ctx2) {
      new Chart(ctx2, {
        ...
        data: {
          labels: vLabels,
          datasets: [
            { label: 'Revenue', data: revenues, backgroundColor: '#34d399', borderRadius: 4 },
            { label: 'Cost', data: costs, backgroundColor: '#f87171', borderRadius: 4 }
          ]
        },
        options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { beginAtZero: true } }, plugins: { legend: { position: 'bottom', labels: { padding: 16, usePointStyle: true } } } }
      });
    }
    ...
    if (ctx3) {
      new Chart(ctx3, {
        type: 'bar',
        data: { labels: vLabels, datasets: [{ label: 'km/L', data: efficiencies, backgroundColor: '#818cf8', borderRadius: 4 }] },
    ...
    const rois = vehicles.map(v => {
      const rev = store.getVehicleRevenue(v.id);
      const c = store.getVehicleOperationalCost(v.id).total;
      return v.acquisitionCost > 0 ? parseFloat((((rev - c) / v.acquisitionCost) * 100).toFixed(1)) : 0;
    });
    ...
    if (ctx4) {
      new Chart(ctx4, {
        type: 'bar',
        data: { labels: vLabels, datasets: [{ label: 'ROI %', data: rois, backgroundColor: roiColors, borderRadius: 4 }] },
        options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { beginAtZero: false } }, plugins: { legend: { display: false } } }
      });
    }
  });
}
...
export function exportReportCSV() {
  const vehicles = store.getVehicles();
  const headers = ['Vehicle', 'Fuel Efficiency (km/L)', 'Revenue', 'Fuel Cost', 'Maint Cost', 'Total Cost', 'ROI %'];
